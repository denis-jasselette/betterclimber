...
<div class="mx-auto max-w-lg px-4 py-8">
	...
	<!-- Grading system ───────────────────────────────────────────────── -->
	<section class="mb-8">
		<h2 class="mb-1 text-xs font-semibold uppercase tracking-wider text-muted">Grading system</h2>
		<p class="mb-3 text-xs text-muted">Controls how grades are displayed everywhere in the app.</p>
		<div class="space-y-2">
			{#each gradingOptions as opt}
				<button
				...
				>
					...
					<div>
						<p class="text-sm font-semibold
							{settings.gradingSystem === opt.value ? 'text-cyan-400' : 'text-text'}">{opt.label}</p>
	...
	<!-- Theme ────────────────────────────────────────────────────────── -->
	<section>
		<h2 class="mb-1 text-xs font-semibold uppercase tracking-wider text-muted">Theme</h2>
		...
		<div class="space-y-2">
			{#each themeOptions as opt}
				...
				>
					...
					<div>
						<p class="text-sm font-semibold
							{settings.theme === opt.value ? 'text-cyan-400' : 'text-text'}">{opt.label}</p>
						<p class="text-xs text-muted">{opt.description}</p>
					</div>
				</button>
			{/each}
