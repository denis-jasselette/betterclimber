<script lang="ts">
	...
	let advancedOpen = $state(filters.minQuality > 0 || filters.onlyBenchmarks || filters.onlyCampus || filters.onlyRoutes);
	const hasAdvancedFilter = $derived(filters.minQuality > 0 || filters.onlyBenchmarks || filters.onlyCampus || filters.onlyRoutes);
	$effect(() => { if (hasAdvancedFilter) advancedOpen = true; });
</script>
...
<div class="space-y-5">
	...
	<div class="relative">
		...
		<input
			...
			class="w-full rounded-xl border border-border bg-surface-raised/60 py-2.5 pl-9 pr-4 text-sm text-text placeholder:text-muted focus:border-cyan-500 focus:outline-none focus:ring-1 focus:ring-cyan-500"
		/>
	</div>
	...
	<!-- User log filters -->
	<div>
		<p class="mb-2 text-xs font-semibold uppercase tracking-wider text-muted">My Sends</p>
			...
			>
				...
				>
					{#if filters.onlyRecentlyLit}
						<svg class="size-3 text-cyan-400" viewBox="0 0 24 24" fill="none" stroke="currentColor" stroke-width="2.5">
	...
	<!-- Advanced filters -->
	<details open={advancedOpen} ontoggle={(e) => (advancedOpen = (e.currentTarget as HTMLDetailsElement).open)} class="group">
		<summary class="flex cursor-pointer select-none list-none items-center gap-1.5 text-xs font-semibold uppercase tracking-wider text-muted hover:text-text">
		...
		<div class="mt-3 space-y-4">
			<!-- Minimum quality -->
			<div>
				<p class="mb-2 text-xs font-semibold uppercase tracking-wider text-muted">Min. Quality</p>
				<div class="flex gap-1.5">
					{#each qualityStars as stars}
						<button
							onclick={() => {
			...
			>
				...
				>
					{#if filters.onlyBenchmarks}
						<svg class="size-3 text-yellow-400" viewBox="0 0 24 24" fill="none" stroke="currentColor" stroke-width="2.5">
							<path d="M5 13l4 4L19 7" />
						</svg>
					{/if}
				</span>
			...
			>
				...
				>
					{#if filters.onlyCampus}
						<svg class="size-3 text-purple-400" viewBox="0 0 24 24" fill="none" stroke="currentColor" stroke-width="2.5">
							<path d="M5 13l4 4L19 7" />
						</svg>
					{/if}
				</span>
			...
			>
				...
				>
					{#if filters.onlyRoutes}
						<svg class="size-3 text-blue-400" viewBox="0 0 24 24" fill="none" stroke="currentColor" stroke-width="2.5">
							<path d="M5 13l4 4L19 7" />
						</svg>
					{/if}
				</span>
